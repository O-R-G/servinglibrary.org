<?php
	...
	$thisitem_name = "Subscription 2-year";
	$thisitem_name_2 = "Subscription 12-year";
	$thispayment_status = "Completed";
	$thispayment_amount = "0.02";
	$thispayment_amount_2 = "0.05";
	$thispayment_currency = "USD";
	$thisreceiver_email = "info-facilitator@servinglibrary.org";	// staging
	// $thisreceiver_email = "info@servinglibrary.org";				// live

	// Validate transaction details against subscription request

	if (	($txn_type == $thistxn_type) && 
		($payment_status == $thispayment_status) && 
		((($payment_amount == $thispayment_amount) &&
		...
		if (($payment_amount == $thispayment_amount) && ($item_name == $thisitem_name)) {

			$dateIncrement = "+2 years";	// 2-year subscription

		} else if (($payment_amount == $thispayment_amount_2) && ($item_name == $thisitem_name_2)) {
